import {
    Box,Flex,Spacer, Grid,GridItem,Heading,Text,Image} from '@chakra-ui/react';
  ...
  <Grid templateColumns='repeat(4, 1fr)' gap={6}>
    ...
    {`${!formData.cardnumber ? "4256 7890 5678 4532" : formData.cardnumber}`}
    </GridItem>
  </Grid>
  ...
  <Flex>
    ...
    <Box p='4' bg='red.400'>
      ...
      {`${!formData.name ? "Neeraj Singh" : formData.name}`}
    </Box>
    <Spacer />
    <Box p='4' bg='green.400'>
      <Text>Expiry Date</Text>
      {`${!(formData.month) ? "10" : formData.month}/`}
      {`${!(formData.year) ? "/30" : formData.year}`}
    </Box>
    <Spacer />
    <Box p='4' bg='green.400'>
      <Text>CVV</Text>
      {`${!formData.cvv ? "669" : formData.cvv}`}
    </Box>
  </Flex>
  </Box>
